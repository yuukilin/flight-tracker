...
- `/add` 新增路線的預設轉機條件是直飛（`max_stops: 0`）；既有路線不自動改動。
...
- `/remove` 已改成不用記 ID；沒帶 ID 時會顯示可點選刪除清單，`/list` 也會直接提供刪除按鈕。
- 所有需要路線 ID 的功能都支援不帶 ID 直接跳選單，包含 `/edit`、`/pause`、`/threshold`、`/history`、`/debug` 等。
...
- `/remove` 不用記 ID；直接輸入 `/remove` 會跳出可點選刪除清單
- 其他需要路線 ID 的指令也不用記 ID；直接輸入 `/edit`、`/history`、`/debug` 等會先跳路線選單
...
- `/add` 預設：不限預算、去回時段不限、直飛（最多轉 0 次），後續可再改。
...
### 2026-06-04 已補完
- `/add` 新增路線預設轉機條件已改成直飛（`max_stops: 0`），確認訊息也改成「預設直飛」。
- 只影響之後新增的路線；既有 `routes.json` 路線設定不自動改。
