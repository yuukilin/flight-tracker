...
def action_history(rid, days, chat_id):
    ...
    if not daily:
        send_text(chat_id, f"📜 #{rid} {name} 過去 {days} 天：無傳統航空資料")
        return

    lines = [f"📜 #{rid} {name} 過去 {days} 天每日最低（傳統航空）："]
...
def action_best(rid, chat_id, limit=5):
    ...
    if not best_rows:
        send_text(chat_id, f"🏆 #{rid} {name} 歷史最低：無資料")
        return

    lines = [f"🏆 #{rid} {name} 歷史最低 {limit} 筆（傳統航空）："]
    for p, an, dd, rd, dest, stops in best_rows:
        s = "直飛" if stops == 0 else f"轉{stops}"
        lines.append(f"  NT$ {p:,} {an} {dd}→{rd} {dest} {s}")
    send_text(chat_id, '\n'.join(lines))
...
def action_chart(rid, days, chat_id):
    ...
    if not daily:
        send_text(chat_id, f"📈 #{rid} {name} 過去 {days} 天：無資料可畫")
        return
